...
export default async (app: Application) => {
  ...
  // declare routes
  app.use(messageValue.main_prefix, routesArray);

  // catch-all routes
  app.get(mS.api_main_slash, (req, res) => {
    res.status(messageValue.ok).send(mS.api_root_response);
  });

  app.all(mS.api_all, (req, res) => {
    res.status(messageValue.not_found).send(mS.non_existing_endpoint);
  });

  app.use((req, res) => {
    res
      .status(messageValue.internal_server_error_code)
      .send(mS.something_went_wrong);
  });
